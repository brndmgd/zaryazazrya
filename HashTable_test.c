...
pool_allocator_t* create_allocator_for_hashtable(size_t bucket_count, size_t n_elements) {
...
void test_hashtable_init() {
    ...
    pool_allocator_t *allocator = create_allocator_for_hashtable(bucket_count, n_elements);
    HashTable table;
...
void test_hashtable_long_key() {
    ...
    char long_key[256];
    for (int i = 0; i < 255; ++i) long_key[i] = 'a';
    long_key[255] = '\0';
